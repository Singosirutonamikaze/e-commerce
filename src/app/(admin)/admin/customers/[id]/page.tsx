  ...
  return (
    <div className="flex flex-col gap-8 p-0 md:p-6">
      <header className="flex items-center gap-4">
        <Link href="/admin/customers" className="h-10 w-10 bg-surface rounded-full flex items-center justify-center shadow-sm border border-border hover:scale-105 transition-transform">
          <ChevronLeft className="h-5 w-5 text-text-primary" />
        </Link>
        <div>
          <h2 className="text-3xl font-black uppercase tracking-tighter text-text-primary flex items-center gap-3">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-1 flex flex-col gap-8">
          <section className="bg-surface rounded-3xl p-8 border border-border shadow-sm flex flex-col items-center text-center relative overflow-hidden">
             <div className="absolute top-0 left-0 w-full h-24 bg-accent/10"></div>
             <div className="h-24 w-24 rounded-full bg-accent text-white flex items-center justify-center font-black text-3xl shadow-xl mt-6 relative z-10 border-4 border-white">
          ...
          <section className="bg-surface rounded-3xl p-6 border border-border shadow-sm flex flex-col gap-4">
            ...
            {customer.addresses && customer.addresses.length > 0 ? (
               <div className="flex flex-col gap-3">
                 {customer.addresses.map((addr) => (
                   <div key={addr.id} className="p-4 bg-surface-alt rounded-2xl border border-border text-xs font-medium text-text-muted">
                  ...
                  {customer.orders.map((order) => (
                    <Link href={`/admin/orders/${order.id}`} key={order.id} className="p-6 flex flex-col sm:flex-row sm:items-center justify-between hover:bg-surface-alt/50 transition-colors border-b border-border last:border-0 group">
                       ...
                       <div className="flex items-center gap-4 mt-3 sm:mt-0">
                          ...
                          <p className="text-sm font-black text-text-primary min-w-[70px] text-right">{formatPrice(Number(order.total))}</p>
