...
import { Users, ChevronRight, Mail, Calendar, UserCheck, ArrowUpRight } from 'lucide-react';
    ...
    <div className="flex flex-col gap-10 max-w-screen-2xl mx-auto">
      ...
      <header className="flex flex-col lg:flex-row lg:items-end justify-between gap-8">
        <div className="flex flex-col gap-3">
          <div className="flex items-center gap-3">
             <span className="text-[10px] font-bold uppercase tracking-[0.3em] text-neutral-400">Relations Clientèle</span>
          </div>
        ...
        <div className="bg-neutral-100 p-1 rounded-sm border border-neutral-200">
           <div className="bg-white px-10 py-3 border border-neutral-100 flex flex-col items-center gap-1">
              <span className="text-[9px] font-bold uppercase tracking-widest text-neutral-400 text-center">Total Enregistrés</span>
              <span className="text-xl font-bold text-black tabular-nums">{customers.length}</span>
           </div>
        </div>
      </header>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-4">
        ...
        <div className="lg:col-span-1">
           <Button variant="outline" className="w-full h-12 rounded-sm border-neutral-200 bg-white hover:border-black transition-all text-[10px] font-bold uppercase tracking-widest">
              Exporter la liste
           </Button>
        </div>
      </div>
      ...
      <Card className="rounded-sm border border-neutral-200 shadow-sm overflow-hidden bg-white">
        {customers.length === 0 ? (
          <div className="py-24 flex flex-col items-center justify-center text-center gap-6 bg-neutral-50/50">
            ...
            <div className="flex flex-col gap-1">
               ...
               <p className="text-[10px] font-medium text-neutral-400 uppercase tracking-widest leading-relaxed">Aucun client n&apos;est encore enregistré dans le système.</p>
            </div>
          </div>
        ) : (
              ...
              >
                <div className="flex items-center gap-6">
                   ...
                   <div className="flex flex-col gap-1">
                     ...
                     <div className="flex items-center gap-4 text-[9px] font-bold uppercase tracking-widest text-neutral-400 mt-1">
                        <span className="flex items-center gap-2 border-b border-transparent hover:border-black transition-colors"><Mail className="h-3 w-3"/> {customer.email}</span>
                        <div className="h-1 w-1 rounded-full bg-neutral-200"></div>
                        <span className="flex items-center gap-2"><Calendar className="h-3 w-3"/> Inscrit le {formatDate(customer.createdAt)}</span>
                     </div>
                   </div>
                </div>
                ...
                <div className="flex items-center gap-10">
                   <div className="flex flex-col items-end gap-1.5">
                       <span className="text-[8px] font-bold uppercase tracking-[0.2em] text-neutral-400">Activité</span>
                       <div className="flex items-center gap-2 px-3 py-1.5 bg-neutral-50 border border-neutral-200 rounded-sm">
                          ...
                          <span className="text-[9px] font-bold text-black uppercase tabular-nums">{(customer as any)._count?.orders || 0} Commandes</span>
                       </div>
                   </div>
        ...
        <div className="p-8 border-t border-neutral-200 bg-neutral-50 flex justify-center">
           <p className="text-[9px] font-bold uppercase tracking-[0.4em] text-neutral-400">Le système privilégie la discrétion et la sécurité des données client.</p>
        </div>
      </Card>
    </div>
  );
