    ...
    <div className="min-h-screen w-full flex flex-col items-center justify-center bg-bg relative overflow-hidden px-6">
      ...
      >
        <div className="relative inline-block mb-12">
          <motion.h1
            initial={{ scale: 0.8 }}
            animate={{ scale: 1 }}
            className="text-[180px] md:text-[240px] font-black leading-none tracking-tighter text-transparent bg-clip-text bg-linear-to-b from-text-primary to-text-muted/10 select-none opacity-20"
          >
            404
          </motion.h1>
          <div className="absolute inset-0 flex items-center justify-center translate-y-10">
            ...
            >
              <Ghost
                className="h-32 w-32 text-accent drop-shadow-2xl"
                strokeWidth={1.5}
              />
        ...
        <div className="space-y-6">
          ...
          <p className="text-lg font-medium text-text-muted max-w-md mx-auto leading-relaxed">
            Désolé, l&apos;adresse que vous recherchez n&apos;existe plus ou a
            été déplacée vers une nouvelle destination.
          </p>
        </div>
      ...
      <div className="hidden lg:block">
        <motion.div
          ...
          className="absolute top-1/4 left-1/4 h-3 w-3 bg-accent/20 rounded-sm"
        ></motion.div>
        <motion.div
          animate={{ y: [0, -30, 0] }}
          transition={{ duration: 5, repeat: Infinity }}
          className="absolute bottom-1/4 right-1/4 h-2 w-2 bg-text-hint/20 rounded-sm"
        ></motion.div>
        <motion.div
          animate={{ x: [0, 40, 0] }}
          transition={{ duration: 4, repeat: Infinity }}
          className="absolute top-2/3 left-1/3 h-4 w-4 bg-accent-light border border-accent/10 rounded-sm"
        ></motion.div>
      </div>
    </div>
  );
