  ...
  return (
    <div className="flex flex-col gap-10">
      <header className="mb-6 flex flex-col gap-4">
        <Link href={ROUTES.ACCOUNT.ORDERS} className="flex items-center gap-2 text-xs font-bold uppercase text-accent hover:text-accent-hover tracking-widest transition-colors mb-2">
        ...
        <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-6">
          ...
          <div className="flex items-center gap-4">
            ...
            <Badge
              ...
              className="font-bold h-8 px-4 rounded-sm uppercase tracking-widest text-[10px]"
            >
              {order.statut}
            </Badge>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
        ...
        <div className="lg:col-span-2 flex flex-col gap-8">
          <section className="bg-surface rounded-sm border border-border overflow-hidden shadow-sm">
            <div className="px-8 py-6 border-b border-border bg-surface-alt/50 flex items-center gap-3">
              ...
              <h3 className="text-sm font-bold uppercase tracking-widest text-text-primary">Articles commandés</h3>
            </div>
              ...
              {order.orderItems.map((item) => (
                <div key={item.id} className="p-8 border-b border-border last:border-0 flex gap-6 group hover:bg-surface-alt/20 transition-all">
                  ...
                  <div className="flex flex-1 flex-col justify-between py-1">
                    ...
                    <p className="text-lg font-bold text-text-primary">{formatPrice(Number(item.prixUnitaire) * item.quantite)}</p>
                  </div>
                </div>
              ))}
            </div>
          ...
          <section className="bg-white rounded-sm p-8 border border-border flex flex-col sm:flex-row items-center gap-8 shadow-sm">
            <div className="h-16 w-16 bg-accent-light rounded-sm flex items-center justify-center text-accent shrink-0">
            ...
            <div>
              ...
              <Link href={ROUTES.ACCOUNT.SUPPORT}>
                <button className="text-xs font-bold text-accent border-b-2 border-accent/20 hover:border-accent transition-all pb-1 uppercase tracking-widest">
                  Besoin d&apos;aide ? →
                </button>
              </Link>
            </div>
        ...
        <div className="lg:col-span-1 flex flex-col gap-10">
          ...
          {/* Order Summary */}
          <section className="bg-surface rounded-sm p-8 border border-border shadow-sm relative overflow-hidden">
            <div className="absolute top-0 right-0 h-24 w-24 bg-accent/5 rounded-sm -translate-y-12 translate-x-12 blur-2xl"></div>

            <div className="flex items-center gap-3 mb-8 relative z-10">
              <CreditCard className="h-5 w-5 text-accent" />
              <h3 className="text-sm font-bold uppercase tracking-widest text-text-primary font-bold tracking-tight">Récapitulatif Financier</h3>
            </div>
            ...
            <div className="flex justify-between items-end">
              <span className="text-sm font-bold text-text-primary uppercase tracking-tighter">Total Payé</span>
              <span className="text-3xl font-bold text-accent">{formatPrice(Number(order.total))}</span>
            </div>

            {order.promo && (
              <p className="mt-6 text-[10px] font-bold uppercase text-success tracking-widest text-center bg-success-bg p-2 rounded-xl">
