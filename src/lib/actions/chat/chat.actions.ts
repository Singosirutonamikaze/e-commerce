...
import { getBotReply, BotResponse } from "./bot";
...
export async function sendMessage(
  ...
  try {
    ...
    // Mise à jour de l'horodatage de la conversation
...
/**
 * Réponse automatisée du Chatbot Conciergerie.
 */
export async function askConciergeBot(
  userQuery: string,
  conversationId?: string,
): Promise<{ success: boolean; botReply: BotResponse; error?: string }> {
  try {
    ...
    if (user) {
      ...
      if (dbUser?.prenom) {
        userName = dbUser.prenom;
      }
    }

    const botReply = getBotReply(userQuery, userName);

    // Si une conversation ID existe et qu'un bot admin peut répondre
    if (conversationId && user) {
      // Optionnel : persistance du message bot dans la conversation si désiré
    }

    return { success: true, botReply };
  } catch (error) {
    console.error("askConciergeBot error:", error);
    return {
      success: true,
      botReply: getBotReply(userQuery),
    };
  }
}
