...
export async function createOrder(data: {
  ...
  try {
    ...
    if (data.promoId) {
      ...
      if (promo?.estActif && promo.dateExpiration > new Date()) {
...
export async function updateOrderStatus(orderId: string, status: OrderStatus) {
  ...
  try {
    ...
    revalidatePath(`${ROUTES.DASHBOARD.ORDERS}/${orderId}`);
    return { success: true };
  } catch (error) {
...
export async function getOrder(id: string) {
  try {
    ...
    return {
      ...
      montantReduction: order.montantReduction
        ? Number(order.montantReduction)
        : 0,
...
export async function getMyOrders() {
  ...
  try {
    ...
    console.error("Erreur lors de la recuperation de mes commandes:", error);
    return [];
  }
}

/**
 * Récupère les commandes (Admin ou utilisateur connecté).
 */
    ...
    if (dbUser?.role === "ADMIN") {
      ...
      const orders = await prisma.order.findMany({
        ...
        include: {
          ...
          orderItems: {
            include: {
              produit: { select: { nom: true, images: { take: 1 } } },
            },
          },
        },
      });
