    ...
    setTimeout(() => {
      setMessages((prev) => [
        ...prev,
        {
          ...
          role: "ADMIN",
          content:
            "Merci pour votre message. Un conseiller va vous répondre sous peu.",
          createdAt: new Date(),
        },
      ]);
    }, 1500);
    ...
    <AnimatePresence>
      {isChatWindowOpen && (
        <motion.div
          ...
          className="fixed bottom-6 right-6 z-100 flex w-100 max-w-[calc(100vw-48px)] flex-col overflow-hidden rounded-sm border border-border bg-surface shadow-2xl"
        >
          {/* Header */}
          <div className="bg-accent p-6 flex items-center justify-between text-white relative h-24 overflow-hidden">
            ...
            <div className="flex items-center gap-4 relative z-10">
              ...
              <div className="flex flex-col">
                <div className="flex items-center gap-2">
                  <h3 className="font-bold text-lg tracking-tighter uppercase">
                    Support Velure
                  </h3>
            ...
            <button
              ...
              className="h-10 w-10 flex items-center justify-center rounded-sm hover:bg-white/10 transition-all text-white/80 hover:text-white relative z-10"
            >
          ...
          <div
            ref={scrollRef}
            className="flex flex-1 min-h-87.5 max-h-112.5 flex-col gap-5 overflow-y-auto bg-surface-alt/20 p-6 scrollbar-hide"
          >
            {messages.map((msg) => (
              <ChatBubble
                key={msg.id}
          ...
          <form
            onSubmit={handleSend}
            className="p-6 bg-white border-t border-border flex items-center gap-3"
          >
