    ...
    <AnimatePresence>
      {isChatWindowOpen && (
        <motion.div
          ...
          className="fixed bottom-6 right-6 z-[100] w-[400px] max-w-[calc(100vw-48px)] flex flex-col bg-surface shadow-[0_20px_60px_-15px_rgba(0,0,0,0.3)] rounded-[32px] border border-border overflow-hidden"
        >
          {/* Header */}
          <div className="bg-accent p-6 flex items-center justify-between text-white relative h-24 overflow-hidden">
             <div className="absolute top-0 right-0 h-24 w-24 bg-white/10 rounded-full -translate-y-12 translate-x-12 blur-2xl"></div>
             <div className="flex items-center gap-4 relative z-10">
                ...
                <div className="flex flex-col">
                   <div className="flex items-center gap-2">
                      <h3 className="font-black text-lg tracking-tighter uppercase">Support Velure</h3>
                      <div className="h-2 w-2 rounded-full bg-success animate-pulse"></div>
                   </div>
                   <p className="text-[10px] font-bold text-white/70 uppercase tracking-widest">En ligne maintenant</p>
                </div>
             </div>
             <button onClick={closeChatWindow} className="h-10 w-10 flex items-center justify-center rounded-2xl hover:bg-white/10 transition-all text-white/80 hover:text-white relative z-10">
          ...
          <form onSubmit={handleSend} className="p-6 bg-white border-t border-border flex items-center gap-3">
             <div className="relative flex-grow">
                <input 
                  ...
                  className="w-full h-12 bg-surface-alt/50 rounded-2xl border border-transparent focus:border-accent focus:bg-white focus:ring-4 focus:ring-accent/5 px-5 text-sm font-bold placeholder:text-text-hint outline-none transition-all"
                />
             ...
             <button 
                ...
                className="h-12 w-12 bg-accent text-white rounded-2xl flex items-center justify-center shadow-lg shadow-accent/20 hover:scale-105 active:scale-95 disabled:grayscale disabled:opacity-50 transition-all shrink-0"
              >
