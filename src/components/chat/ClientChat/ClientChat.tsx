...
export function ClientChat({ conversationId }: Readonly<ClientChatProps>) {
  ...
  return (
    <div className="flex flex-col h-150 bg-white rounded-sm border border-border shadow-2xl overflow-hidden">
      {/* Header Info */}
      <div className="px-8 py-6 bg-surface-alt/50 border-b border-border flex items-center justify-between">
        <div className="flex items-center gap-4">
          <div className="h-10 w-10 bg-accent text-white rounded-sm flex items-center justify-center font-bold">
            V
          </div>
          <div className="flex flex-col">
            <span className="text-sm font-bold text-text-primary uppercase tracking-tighter italic">
              Support Velure
            </span>
      ...
      >
        {messages.length > 0 ? (
          ...
          <div className="flex-1 flex flex-col items-center justify-center text-center p-12 opacity-40">
            ...
            <p className="text-xs font-bold uppercase tracking-[0.2em] text-text-hint mb-2">
      ...
      <form
        onSubmit={handleSend}
        className="p-8 bg-white border-t border-border"
      >
        <div className="relative flex items-center gap-4 bg-surface-alt px-4 py-2 rounded-sm border border-transparent focus-within:border-accent focus-within:bg-white focus-within:ring-4 focus-within:ring-accent/5 transition-all">
          <input
            ...
            className="grow h-12 bg-transparent outline-none text-sm font-bold placeholder:text-text-hint/70"
          />
          <Button
            type="submit"
