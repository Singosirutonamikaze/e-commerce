    ...
    <form onSubmit={handleSubmit(onSubmit)} className="grid grid-cols-1 lg:grid-cols-3 gap-10">
      <div className="lg:col-span-2 flex flex-col gap-8">
        <section className="bg-surface rounded-sm p-8 border border-border shadow-sm flex flex-col gap-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-2">
              <label className="text-xs font-bold uppercase tracking-widest text-text-hint px-1">Code Promo (Unique)</label>
              <Input {...register('code')} error={!!errors.code} placeholder="SUMMER25" className="h-12 rounded-sm uppercase" />
            </div>

            <div className="space-y-2">
              <label className="text-xs font-bold uppercase tracking-widest text-text-hint px-1">Type de réduction</label>
              <select
                {...register('type')}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-2">
              <label className="text-xs font-bold uppercase tracking-widest text-text-hint px-1">
                Valeur ({promoType === 'POURCENTAGE' ? '%' : '€'})
              </label>
              <Input type="number" step="0.01" {...register('reduction', { valueAsNumber: true })} error={!!errors.reduction} className="h-12 rounded-sm" />
            </div>

            <div className="space-y-2">
              <label className="text-xs font-bold uppercase tracking-widest text-text-hint px-1">Seuil minimal d'achat (€)</label>
              <Input type="number" step="0.01" {...register('montantMinimum', { valueAsNumber: true })} error={!!errors.montantMinimum} className="h-12 rounded-sm" />
            </div>
          </div>
        </section>
        ...
        <section className="bg-surface rounded-sm p-8 border border-border shadow-sm flex flex-col gap-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-2">
              <label className="text-xs font-bold uppercase tracking-widest text-text-hint px-1">Date d'expiration</label>
              <Input type="date" {...register('dateExpiration')} error={!!errors.dateExpiration} className="h-12 rounded-sm" />
            </div>

            <div className="space-y-2">
              <label className="text-xs font-bold uppercase tracking-widest text-text-hint px-1">Limit d'utilisation (Optionnel)</label>
              <Input type="number" {...register('limiteUtilisation', { valueAsNumber: true })} error={!!errors.limiteUtilisation} placeholder="Laisser vide pour illimité" className="h-12 rounded-sm" />
            </div>
          </div>
        </section>
      </div>
      ...
      <div className="lg:col-span-1 flex flex-col gap-8">
        <section className="sticky top-24 bg-surface rounded-sm p-8 border border-border shadow-sm flex flex-col gap-6">
          <h3 className="text-xs font-bold uppercase tracking-widest text-text-hint">Activer la Promo</h3>
