...
export function CartSummary({ onCheckout }: Readonly<CartSummaryProps>) {
  ...
  const shippingProgress = Math.min((subTotal / 100000) * 100, 100);
  const shippingProgressClass = getShippingProgressClass(shippingProgress);
  ...
  return (
    <div className="flex flex-col gap-8">
      <Card className="p-8 border-border shadow-xl rounded-sm sticky top-32 overflow-hidden bg-white">
        ...
        <h2 className="text-xl font-bold text-text-primary uppercase tracking-tighter mb-8 relative z-10">
        ...
        <div className="space-y-4 mb-8">
          ...
          <div className="mt-4 pt-4 border-t border-dashed border-border">
            ...
            <div className="h-1 w-full bg-neutral-100 mt-2 rounded-sm overflow-hidden">
              <div
                className={`h-full bg-black transition-all duration-1000 ${shippingProgressClass}`}
              ></div>
            </div>
            {subTotal < 100000 && (
        ...
        <div className="mt-10 flex flex-col gap-4">
          <div className="flex items-center gap-3">
            ...
            <span className="text-xs font-bold text-text-muted tracking-tight">
      ...
      <div className="bg-surface-alt rounded-sm p-6 border border-border flex items-center justify-between group cursor-pointer hover:border-accent/40 transition-all">
        <div>
          ...
          <p className="text-xs text-text-hint font-medium">
